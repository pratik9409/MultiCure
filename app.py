...
def resultc():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resultc.html', filename=filename, fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resultbt():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resultbt.html', filename=filename, fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resultd():
    ...
    return render_template('resultd.html', fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resultbc():
    if request.method == 'POST':
        ...
        return render_template('resultbc.html', fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resulta():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resulta.html', filename=filename, fn=firstname, ln=lastname, age=age, gender=gender, r=0)
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if pred < 0.5:
                ...
                pred = 1
            # pb.push_sms(pb.devices[0],str(phone), 'Hello {},\nYour COVID-19 test results are ready.\nRESULT: {}'.format(firstname,['POSITIVE','NEGATIVE'][pred]))
            return render_template('resultp.html', filename=filename, fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resulth():
    if request.method == 'POST':
        ...
        # pb.push_sms(pb.devices[0],str(phone), 'Hello {},\nYour Diabetes test results are ready.\nRESULT: {}'.format(firstname,['NEGATIVE','POSITIVE'][pred]))
        return render_template('resulth.html', fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
...
def resultm():
    if request.method == 'POST':
        ...
        return render_template('resultm.html', fn=firstname, ln=lastname, age=age, gender=gender, r=pred)
    else:
        flash('Allowed image types are - png, jpg, jpeg')
        return redirect(request.url)
